...
use Satisfactory\SettingBundle\Form\SegmentType;
...
class SegmentController extends Controller
{
    ...
    public function newAction(Request $request)
    {
        ...
        $segments = $em->getRepository('SatisfactorySettingBundle:Segment')->findAll();
        $segment = new Segment();
        $form = $this->createForm('Satisfactory\SettingBundle\Form\SegmentType', $segment);
        $form->handleRequest($request);
        
        if ($form->isSubmitted() && $form->isValid()) {
            if ($segment->checkFile() == NULL) {
                $em->createQuery('DELETE FROM Satisfactory\SettingBundle\Entity\Segment')->execute();
                $segment->upload();
                // open csv 
                ...
                if (($handle = fopen($segment->getUploadRootDir()."/".$segment->getPath(), "r")) !== FALSE) {
                    while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
                        ...
                        for ($c = 0; $c < $num; $c++) {
                            ...
                            if ($tab[0] <> 'id_segment') {
                                $segment = new Segment();
                                $segment->setSegmentId($tab[0]);
                                $segment->setName($tab[1]);
        ...
        return $this->render('SatisfactorySettingBundle:Segment:new.html.twig', array(
            'countSegment' => $segments ? count($segments): 0,
