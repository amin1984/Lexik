...
class DealingExecuteForTestCommand extends ContainerAwareCommand
{
    private $em;
    ...
    {
        $this->setName("cron:dealingExecuteForTest")
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
         ...
         * @var Finder $files
         */
        $files = $extractor->extractFromFile('/var/www/html/outilsatisfactory/web/uploads/test.rar');
        echo "rar \n\n\n\n\n\n\n";
    ...
    protected function executeOperation($dealing,$operation,$tmpData,$operationNum,$log = null) 
    {
        ...
        switch ($operation->getType()) {
            ...
            case "Concat" : //Table de correspondance
                $resultFile = $this->executeOperationConcatener($dealing,$operation,$tmpData,$operationNum,$log);
            ...
            case "6": //Modifier le format d'une colonne
                ...
                switch ($operation->getColumnFormat()) {
                    case 1: //Format telephone
                        ...
                        if($operation->getTargetFormatPhone() == 2) 
...
//                $dm->persist($document);
    ...
    protected function executeOperationConcatener($dealing,$operation,$tmpData,$operationNum,$log = null)
    {
        if(!$operation || !$dealing || !$tmpData) {
            return false;
        }
        ...
        $ConcatFtp = $em->getRepository('SatisfactoryOperationBundle:Concat')->findBy(array('operation'=>$operation));
