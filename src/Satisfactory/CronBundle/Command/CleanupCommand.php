...
class CleanupCommand extends ContainerAwareCommand
{
    ...
    {
        $this->setName("staisfactory:cleanup")
        ->setDescription("Cleanup table EnterCorrespondence");
    }

    protected function execute(InputInterface $input, OutputInterface $output)
    {
        $this->manualexecuteAction(152);
        ...
        switch ($operation->getEnrichFilter()) {
            case 1 : // Appliquer une correspondance de valeurs sur une colonne :
                ...
                while( $count >= $offset )
                {
                    ...
                    while( $fullThreshold >= $tempOffset && $count >= $offset)
                    {
                        $entries = $repo->getPaginatedResults( $threshold, $offset, $correspondance->getId());

                        foreach($entries as $entry)
                        {
                            $computed[] = $entry->getInput();
                        }
