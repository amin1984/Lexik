...
use Satisfactory\CronBundle\Repository\MimeMailParserAttachment;
/**
...
class MimeMailParser {
         ...
         * @param $type Object[optional]
         */
        public function getAttachments($log) {
//            fwrite($log, "exists 3-1 \n");  
                $attachments = array();
                $dispositions = array("attachment","inline");
                foreach($this->parts as $part) {
//                        fwrite($log, "---exists 3-2 \n");  
                        $disposition = $this->getPartContentDisposition($part);
//                        fwrite($log, "---exists 3-3 \n");  
                        if (in_array($disposition, $dispositions)) {
//            fwrite($log, "---exists 3-4 \n"); 
            if(class_exists('MimeMailParser_attachment'));
//                fwrite($log, "exists MimeMailParser_attachment \n");  
//            fwrite($log, $part['disposition-filename']." \n");  
//            fwrite($log, $this->getPartContentType($part,$log)." \n");  
//            fwrite($log, $this->getAttachmentStream($part,$log)." \n");  
//            fwrite($log, " aaaaaaaaaaaaaaaaaaaaa\n");  
//            fwrite($log, " bbbbbbbbbbbbbbbbbbbbbbbb \n");  
                                $attachments[] = new MimeMailParserAttachment(
                                        $part['disposition-filename'], 
                                        $this->getPartContentType($part,$log), 
                                        $this->getAttachmentStream($part,$log),
                                        $disposition,
                                        $this->getPartHeaders($part),
                                        $log
                                );
            fwrite($log, "---exists 3-5 \n");  
                        }
                }
                return $attachments;
        }
         ...
         * @param $part Array
         */
        private function getPartContentType($part,$log) {
         ...
         * @param $part Array
         */
        private function getAttachmentStream(&$part,$log) {
            fwrite($log, "----------exists 3-4-1111 \n");  
                $temp_fp = tmpfile();   
            fwrite($log, "----------exists 3-4-222222 \n");  
